...
var (
	...
	maxBody     = flag.Int64("maxbody", 50, "max request body size in MB (upload + editor, 0 = no limit)")
...
func main() {
	...
	for _, ip := range localIPs() {
		...
		if useTLS {
			log.Printf("  https://%s", net.JoinHostPort(ip, port[1:])+basePath)
		} else {
			log.Printf("  http://%s", net.JoinHostPort(ip, port[1:])+basePath)
		}
	}
